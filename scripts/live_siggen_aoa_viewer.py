...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Signal-generator AoA viewer using dual-channel phase difference."
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to current_phase_offset.json. "
            "If provided, this overrides --aoa-phase-offset-rad."
        ),
    )
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional gain phase delta table JSON. "
            "Used with --current-gain or --gain to update phase offset when gain changes."
        ),
    )
...
def setup_phase_calibration_runtime(args: argparse.Namespace):
    """
    current_phase_offset.json / gain_phase_table.json을 읽어서
    실시간 AoA에 적용할 phase offset을 args.aoa_phase_offset_rad에 반영한다.
    """
    phase_json = getattr(args, "aoa_phase_calibration_json", None)
    gain_table = getattr(args, "aoa_gain_phase_table", None)
    ...
    try:
        ...
        # viewer마다 gain 인자 이름이 다를 수 있어서 넓게 대응
        for name in ("current_gain", "gain", "rx_gain"):
        ...
        if gain_table and current_gain is not None:
            state = resolve_phase_offset_to_apply(
                current_phase_path=phase_json,
                gain_table_path=gain_table,
        ...
        # 기존 viewer가 쓰는 phase offset 인자에 주입
        args.aoa_phase_offset_rad = float(state.phase_offset_to_apply_rad)
        ...
        print(f"[WARN] Failed to setup phase calibration runtime: {exc}")
        return None
...
def empty_aoa_features(args: argparse.Namespace) -> dict[str, Any]:
    return {
        ...
        "aoa_phase_calibration_json": getattr(args, "aoa_phase_calibration_json", None),
        "aoa_gain_phase_table": getattr(args, "aoa_gain_phase_table", None),
...
def should_compute_aoa(
    ...
    """
    Signal-generator AoA gate.

    This viewer is for CW/signal-generator testing, not drone CNN decision.
    Therefore AoA is NOT gated by CNN raw/final decision.

    We still block obvious overload, but otherwise try AoA calculation and let
    compute_aoa_features() decide reliability using channel coherence.
    """
...
def print_update(row: dict[str, Any]) -> None:
    print(
        ...
        "SR={signal_ratio:.3f} raw_peak={raw_peak:.6g} "
        "clip={clip_ratio:.6g} "
        "AoA={aoa_deg:.2f} AoA_sm={aoa_deg_smooth:.2f} "
        "phase={aoa_phase_diff_rad:.3f} coh={aoa_coherence:.3f} "
        "aoa_status={aoa_status} smooth={aoa_smooth_status}".format(**row),
        flush=True,
    )
...
def main() -> None:
    ...
    print("=== Signal Generator AoA Viewer ===")
    ...
    print("mode note          : siggen AoA ignores CNN drone gate; use --decision-mode none")
