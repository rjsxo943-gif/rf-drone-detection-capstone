...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--distance-step-m", type=float, default=3.0)
    p.add_argument("--true-angle-deg", type=float, default=0.0)
    p.add_argument("--angle-step-deg", type=float, default=5.0)
    p.add_argument("--capture-trusted-n", type=int, default=30)
...
def build_overlay(
    ...
    return [
        ...
        f"[LABEL] d={state.distance_m:.1f}m angle={float(getattr(state, 'true_angle_deg', 0.0)):.1f}deg",
        f"[PHASE] live_delta={float(getattr(state, 'phase_offset_live_delta_deg', 0.0)):.1f}deg total={float(getattr(state, 'phase_offset_total_deg', 0.0)):.1f}deg",
        ...
        "q quit | p pause | [/] gain | 1/2 dist | a/d angle | ,/. phase | m reset | s capture | x cancel",
    ]
...
def run() -> int:
    ...
    capture_target = max(1, int(args.capture_trusted_n))
    ...
    try:
        while state.running:
            ...
            if capture_active:
                ...
                if capture_ok:
                    capture_saved_count += 1
                    capture_row = dict(row)
                    capture_row.update(
                        {
                            "capture_active": True,
                            "capture_saved_index": int(capture_saved_count),
                            "capture_target": int(capture_target),
                            "capture_done": bool(capture_saved_count >= capture_target),
                        }
                    )
                    append_sector_profile(profile_csv, capture_row)

                    print(
                        f"[CAPTURE] saved {capture_saved_count}/{capture_target} "
                    ...
                    if capture_saved_count >= capture_target:
                        capture_active = False
            ...
            capture_label = (
                f"[CAPTURE] active {capture_saved_count}/{capture_target}"
                if capture_active
                else f"[CAPTURE] idle target={capture_target} | s start | x cancel"
            )
            overlay.insert(max(0, len(overlay) - 1), capture_label)
            ...
            if key == "quit":
                ...
                capture_active = False
                print(f"[CAPTURE] canceled at {capture_saved_count}/{capture_target}", flush=True)
            elif key == "save_profile":
                capture_active = True
                capture_saved_count = 0
                capture_target = max(1, int(args.capture_trusted_n))
                print(
                    f"[CAPTURE] start trusted-only capture target={capture_target} "
                    f"d={state.distance_m:.1f}m "
                    f"angle={float(getattr(state, 'true_angle_deg', 0.0)):.1f}deg",
                    flush=True,
                )
