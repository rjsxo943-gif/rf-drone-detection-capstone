from __future__ import annotations

from dataclasses import dataclass


@dataclass
class AoAComputeGateResult:
    """
    AoA 계산 여부 판단 결과.
    """

    should_compute: bool
    reason: str
    class_name: str
    confidence: float
    target_class: str
    confidence_threshold: float


def normalize_class_name(class_name: str) -> str:
    """
    class 이름 표기를 내부 표준 형식으로 맞춘다.

    예:
    "Drone-like"  -> "drone_like"
    "drone like"  -> "drone_like"
    "DRONE_LIKE"  -> "drone_like"
    """
    return (
        str(class_name)
        .strip()
        .lower()
        .replace("-", "_")
        .replace(" ", "_")
    )


class AoAComputeGate:
    """
    CNN 분류 결과를 기준으로 AoA 계산 여부를 결정한다.

    현재 정책:
    - class가 drone_like
    - confidence가 threshold 이상
    - 위 조건을 만족할 때만 AoA 계산 진행
    """

    def __init__(
        self,
        target_class: str = "drone_like",
        confidence_threshold: float = 0.85,
        enabled: bool = True,
    ) -> None:
        self.target_class = normalize_class_name(target_class)
        self.confidence_threshold = float(confidence_threshold)
        self.enabled = bool(enabled)

        if not 0.0 <= self.confidence_threshold <= 1.0:
            raise ValueError(
                f"confidence_threshold must be between 0 and 1. "
                f"got {self.confidence_threshold}"
            )

    def apply(
        self,
        class_name: str,
        confidence: float,
    ) -> AoAComputeGateResult:
        normalized_class_name = normalize_class_name(class_name)
        confidence = float(confidence)

        if not self.enabled:
            return AoAComputeGateResult(
                should_compute=True,
                reason="gate_disabled",
                class_name=normalized_class_name,
                confidence=confidence,
                target_class=self.target_class,
                confidence_threshold=self.confidence_threshold,
            )

        if normalized_class_name != self.target_class:
            return AoAComputeGateResult(
                should_compute=False,
                reason=f"class_is_not_{self.target_class}",
                class_name=normalized_class_name,
                confidence=confidence,
                target_class=self.target_class,
                confidence_threshold=self.confidence_threshold,
            )

        if confidence < self.confidence_threshold:
            return AoAComputeGateResult(
                should_compute=False,
                reason="confidence_too_low",
                class_name=normalized_class_name,
                confidence=confidence,
                target_class=self.target_class,
                confidence_threshold=self.confidence_threshold,
            )

        return AoAComputeGateResult(
            should_compute=True,
            reason="passed",
            class_name=normalized_class_name,
            confidence=confidence,
            target_class=self.target_class,
            confidence_threshold=self.confidence_threshold,
        )


def should_compute_aoa(
    class_name: str,
    confidence: float,
    target_class: str = "drone_like",
    confidence_threshold: float = 0.85,
) -> bool:
    """
    AoA 계산을 수행할지 True/False로 간단히 판단한다.
    """
    gate = AoAComputeGate(
        target_class=target_class,
        confidence_threshold=confidence_threshold,
        enabled=True,
    )

    return gate.apply(
        class_name=class_name,
        confidence=confidence,
    ).should_compute